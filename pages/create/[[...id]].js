...
const ProductManager = () => {
	...
	const handlerSubmit = async (e) => {
		...
		let res;
		if(onEdit){
			res = await putData(`product/${id}`, {...product, images: [...imgOldURL, ...media]}, auth.toke)
			if(res.err) return dispatch({type: 'NOTIFY', payload: {error: res.err}})
		}else{
			res = await postData('product', {...product, images: [...imgOldURL, ...media]}, auth.toke)
			if(res.err) return dispatch({type: 'NOTIFY', payload: {error: res.err}})
		}
	...
	return (
		<div className="products_manager" style={{overflow: 'hidden'}}>
			<Head>
				<title>Create Product</title>
			</Head>
			<form className="row" style={{padding: "0 15px"}} onSubmit={handlerSubmit}>
				<div className="col-md-6">
					<input type="text" name="title" value={title} placeholder="Title" 
					className="border border-dark d-block my-4 w-100 p-2"
				...
				<div className="row">
					...
					<div className="col-sm-6">
						...
						<input type="number" name="inStock" value={inStock} placeholder="InStock" 
						className="border border-dark d-block my-4 w-100 p-2"
						onChange={handleChangeInput}
						/>
					</div>
				...
				<button type="submit" className="btn btn-info my-2 px-4" style={{margin: "0 15px",width:"98%"}}>
